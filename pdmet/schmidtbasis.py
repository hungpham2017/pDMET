...
def get_bath_using_RHF_1RDM(supercell_1RDM, imp_indices=None, is_ROHF=False, num_bath=None, bath_truncation=True, threshold=1.e-10):
    '''
    ...
    if imp_indices is None:
        ...
        Nenv = Nlo - Nimp
        imp_imp_mask = np.matrix(imp_indices).T.dot(np.matrix(imp_indices)) == 1
        imp_emb_1RDM = supercell_1RDM[0][imp_imp_mask].reshape(Nimp, Nimp)
        env_env_mask = env_indices.T.dot(np.matrix(env_indices)) == 1
        env_emb_1RDM = supercell_1RDM[0][env_env_mask].reshape(Nenv, Nenv)  
        ...
        elif not bath_truncation and num_bath is None:
            Nbath = Nimp
        elif num_bath is not None: 
            Nbath = num_bath   
        if Nimp + Nbath > Nlo:
            Nbath = Nlo - Nimp
...
def get_bath_using_gamma_RHF_1RDM(supercell_1RDM, imp_indices=None, threshold=1.e-10):
    ...
    # For the impurity:
    imp_imp_mask = np.matrix(imp_indices).T.dot(np.matrix(imp_indices)) == 1
    imp_1RDM = supercell_1RDM[0][imp_imp_mask].reshape(Nimp, Nimp) 
