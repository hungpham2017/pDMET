...
def get_wannier(w90, supercell = [1,1,1], grid = [50,50,50]):
    ...
    for k_id in range(w90.num_kpts_loc):
        ...
        C_opt = mo_included[:,mo_in_window].dot(w90.U_matrix_opt[k_id][ :, mo_in_window].T)                 
...
def plot_mo_gamma(mf, outfile='mo', grid=[50,50,50]):
    '''
    Export crystalline orbitals from a Gamma-point calculation
    xsf format: http://web.mit.edu/xcrysden_v1.5.60/www/XCRYSDEN/doc/XSF.html
    Attributes:
        wf_list		: a list of MLWFs to plot
        supercell	: a supercell used for plotting
    ...
    for i, mo in enumerate(mos):
        ...
        with open(outfile + '-' + str(i) + '.xsf', 'w') as f:
            f.write('Generated by the pyWannier90\n\n')		
            f.write('CRYSTAL\n')
            f.write('PRIMVEC\n')	
            for row in range(3):
            ...
            for iz in range(nz):
                for iy in range(ny):
                    f.write(fmt % tuple(mo[:,iy,iz].tolist()))										
            f.write('END_DATAGRID_3D\nEND_BLOCK_DATAGRID_3D')
